...
public class DragDropScreen {
	...
	public String dragBoton() {
		...
		MobileElement txtDropHere= (MobileElement) this.newappdriver.findElement(rec_dropZone.xpath("//*[@text=\""+"Drop here."+"\"]"));
			
		int var_DragMe_X = this.newappdriver.findElement(btn_DragMe).getLocation().getX();
		int var_DragMe_Y = this.newappdriver.findElement(btn_DragMe).getLocation().getY();
		int var_DropHere_X = txtDropHere.getLocation().getX();
		int var_DropHere_Y = txtDropHere.getLocation().getY();
		int var_Ancho = this.newappdriver.findElement(btn_DragMe).getSize().width;
		
		System.out.println(var_DragMe_X + " - " + var_DragMe_Y + " - " + var_Ancho);
		
		@SuppressWarnings("rawtypes")
		TouchAction action = new TouchAction(this.newappdriver);
		...
		System.out.println(this.newappdriver.findElement(lbl_Success).getText());
		
		return this.newappdriver.findElement(lbl_Success).getText();
	}
